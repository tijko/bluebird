...
class Bluebird(object):
    ...
    def make_syscall(self):
        # Opcode writes will have to be on WordSize alignments
        # because of how ptrace operates...
        #
        # This is in string-form but passing the integer value
        # of the opcode should...work??
        #opcode = '0f05000000000000'
        # replace addr
        opcode = 3845
        self.create_mmap(0, PAGESIZE, PROT_EXEC | PROT_WRITE, 
                                     MAP_SHARED | MAP_ANONYMOUS, 0)
        # read in /proc smaps?
        # XXX tmp instantiate attr
        self.map_start = int(self.maps['(deleted)'][0][0], 16)
        # will this endian need to be adjusted?
        self.write(self.map_start, opcode)
        callmap(self.traced_pid, self.map_start)
        # mmap page (exec)
        # writenum call 0f 05 to addr
        # call the new func with addr of above
